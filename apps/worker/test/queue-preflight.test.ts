 ...
 * `agentSandbox`/`mailboxSync`/`sendExecute` and Phase 4's three `knowledge.*` queues unconditionally
 * at boot, before any role-gated `registerJob` call —
...
describe('worker boot: pre-created queues accept sends with no registrations', () => {
  ...
  it.each([JOB_NAMES.ticketDraft, JOB_NAMES.sendExecute, JOB_NAMES.agentSandbox, JOB_NAMES.notifyDispatch, JOB_NAMES.knowledgeIngest, JOB_NAMES.knowledgeCrawl, JOB_NAMES.knowledgeEmbedBatch])('pre-creating %s carries policy short', async (name) => {
    const boss: PgBoss = await startTestBoss()
