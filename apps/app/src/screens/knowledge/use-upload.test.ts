...
let mockDeleteSourceImpl: (input: unknown) => Promise<unknown> = (input) => {
  mockDeleteSourceCalls.push(input)
  mockCallOrder.push('delete')
  return Promise.resolve({ ok: true })
}
...
test('a failed PUT deletes the row startUpload minted, so no `queued` source is left with no job coming', async () => {
  mockUploadImpl = (input) => {
    mockUploadCalls.push(input)
  ...
  expect(mockDeleteSourceCalls).toEqual([{ sourceId: 's1' }])
  expect(mockCompleteUploadCalls).toHaveLength(0)
  expect(withoutId(result.current.pending)).toEqual([{ name: 'doc.pdf', progress: 'failed', reason: 'upload_failed' }])
})

test('a failed completeUpload deletes the row too', async () => {
  mockCompleteUploadImpl = (input) => {
    mockCompleteUploadCalls.push(input)
...
test('a deleteSource that itself fails is swallowed — the owner still sees the upload failure', async () => {
  mockUploadImpl = (input) => { mockUploadCalls.push(input); return Promise.reject(new Error('network blip')) }
  mockDeleteSourceImpl = (input) => { mockDeleteSourceCalls.push(input); return Promise.reject(new Error('also down')) }
  const { result } = await setup()

  let outcome
  await act(async () => { outcome = await result.current.start([file()]) })
...
test('a FORBIDDEN from startUpload deletes nothing — no row was minted', async () => {
  mockStartUploadImpl = (input) => {
    mockStartUploadCalls.push(input)
    return Promise.reject({ data: { code: 'FORBIDDEN' }, message: 'knowledge.max_sources reached (2)' })
  }
  const { result } = await setup()
