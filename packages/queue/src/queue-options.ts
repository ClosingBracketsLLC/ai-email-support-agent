 ...
 * through `enqueue`'s `debounceSeconds` instead — CLAUDE.md, Jobs). Where a value already lives as a
 * named constant this package CAN import (`@aesa/core`'s `INVARIANTS` — `ticketDraft`/`agentSandbox`'s
 * `DRAFT_JOB_EXPIRE_SECONDS`, `sendExecute`'s `SEND_QUEUE_EXPIRE_SECONDS`/`SEND_RETRY_DELAY_SECONDS`),
 * it is referenced, not copied as a literal, so a future change to `invariants.ts` propagates here
 * automatically. `knowledgeCrawl`'s `retryDelay` is the one exception: `CRAWL_LEASE_SECONDS` lives in
 * a worker job file this package cannot import without inverting the dependency direction, so that
 * one value is duplicated by hand with a keep-in-sync comment at both ends.
 */
export const QUEUE_OPTIONS: Record<JobName, JobQueueOptions> = {
  ...
  [JOB_NAMES.ticketDraft]: { policy: 'short', expireInSeconds: INVARIANTS.DRAFT_JOB_EXPIRE_SECONDS, retryLimit: 1, retryDelay: 30, retryBackoff: true },
  [JOB_NAMES.agentSandbox]: { policy: 'short', expireInSeconds: INVARIANTS.DRAFT_JOB_EXPIRE_SECONDS, retryLimit: 0 },
  [JOB_NAMES.sendExecute]: {
