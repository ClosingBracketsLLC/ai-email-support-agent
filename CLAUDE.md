...
## Commands
    ...
    pnpm db:up                                    # Postgres 17 + pgvector on :5434 (aesa/aesa/aesa_dev) (pgvector is installed by db-init; a volume from before Phase 4 needs `pnpm db:down && pnpm db:up`); also brings up minio on :9000/:9001
    pnpm s3:init                                  # creates the dev bucket + CORS policy in minio (idempotent; safe to re-run)
